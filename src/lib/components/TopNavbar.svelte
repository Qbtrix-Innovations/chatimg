...
<nav class={clsx('w-full flex flex-row justify-between   py-2 px-4')}>
	<div class={clsx('flex   items-center')}>
		...
		<div class={clsx('flex flex-col')}>
			<div
				class={clsx(
					'text-transparent text-sm p-0 m-0 bg-clip-text bg-gradient-to-t from-[rgba(0,217,24,1)] to-[rgba(0,169,19,1)]'
				)}
			>
				{decodeURIComponent(headingLeft)}
			</div>
			<div class={clsx('flex flex-row justify-start align-middle items-center ')}>
				<div class={clsx('text-[10px] opacity-70 leading-3 text-[rgba(38,50,56,1)] font-medium')}>
					{creditsLeft} Credits left
				</div>
				<div
					class={clsx(
						'text-[8px] leading-[10px] font-semibold text-[rgba(125,125,125,1)] bg-[rgba(217,217,217,1)] rounded-sm ml-1 px-[5px]'
					)}
				>
					{passType}
				</div>
	...
	<div class={clsx('flex flex-row justify-around align-middle items-center pr-2')}>
		...
		<div class={clsx('flex items-center align-middle')}>
			<PrimaryButton onClick={handleRightClick} class={clsx('h-7')} text={buttonTextRight} />
		</div>
	</div>
</nav>
